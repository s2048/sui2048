...
const constructTransaction = (direction, activeGameAddress, contractAddress) => {
  ...
  transactionBlock.moveCall({
    target: `${contractAddress}::game_2048::make_move`,
    arguments: [
...
const execute = async (
  ...
  if (
    (effects?.status?.error || "").indexOf(
      'name: Identifier("game_board_2048") }, function: 17, instruction: 8 }, 3)'
    ) > -1
  ) {
    onError({ gameOver: true });
    return;
  ...
  const event = events.find((e) => e.type === `${contractAddress}::game_2048::GameMoveEvent2048`)
