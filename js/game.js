...
function handleResult(newBoard, direction) {
  if (newBoard.topTile > topTile) {
    ...
    for (const topTile of topTiles) {
      topTile.innerHTML = `<img src='https://raw.githubusercontent.com/s2048/sui2048/main/images/${newBoard.topTile}.png' />`;
    }
    confetti.run();
      ...
      if (mint.innerHTML.indexOf(mintButtonTitle) === -1) {
        ...
        setOnClick(mintButton, async () => {
          ...
          transactionBlock.moveCall({
            target: `${contractAddress}::game_2048::create`,
            typeArguments: [],
          ...
          try {
            ...
            const { events } = data;
            const gameData = events.find((e) => e.type === `${contractAddress}::game_2048::NewGameEvent2048`)
