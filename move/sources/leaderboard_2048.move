module ethos::leaderboard_2048 {
    ...
    use ethos::game_2048::{Self, Game2048};
    ...
    struct Leaderboard2048 has key, store {
        id: UID,
    ...
    struct TopGame2048 has store, copy, drop {
        game_id: ID,
    ...
    public entry fun create(ctx: &mut TxContext) {
        let leaderboard = Leaderboard2048 {
            id: object::new(ctx),
    ...
    public entry fun submit_game(game: &mut Game2048, leaderboard: &mut Leaderboard2048) {
        let top_tile = *game_2048::top_tile(game);
        let score = *game_2048::score(game);

        assert!(top_tile >= leaderboard.min_tile, ELowTile);
        assert!(score > leaderboard.min_score, ELowScore);

        let leader_address = *game_2048::player(game);
        let game_id = game_2048::id(game);

        let top_game = TopGame2048 {
            game_id,
            leader_address,
            score: *game_2048::score(game),
            top_tile: *game_2048::top_tile(game)
        };
    ...
    public fun game_count(leaderboard: &Leaderboard2048): u64 {
        vector::length(&leaderboard.top_games)
    }

    public fun top_games(leaderboard: &Leaderboard2048): &vector<TopGame2048> {
        &leaderboard.top_games
    }

    public fun top_game_at(leaderboard: &Leaderboard2048, index: u64): &TopGame2048 {
        vector::borrow(&leaderboard.top_games, index)
    }

    public fun top_game_at_has_id(leaderboard: &Leaderboard2048, index: u64, game_id: ID): bool {
        let top_game = top_game_at(leaderboard, index);
        top_game.game_id == game_id
    }

    public fun top_game_game_id(top_game: &TopGame2048): ID {
        top_game.game_id
    }

    public fun top_game_top_tile(top_game: &TopGame2048): &u64 {
        &top_game.top_tile
    }

    public fun top_game_score(top_game: &TopGame2048): &u64 {
        &top_game.score
    }

    public fun min_tile(leaderboard: &Leaderboard2048): &u64 {
        &leaderboard.min_tile
    }

    public fun min_score(leaderboard: &Leaderboard2048): &u64 {
        &leaderboard.min_score
    }

    fun add_top_game_sorted(leaderboard: &mut Leaderboard2048, top_game: TopGame2048) {
    ...
    public(friend) fun merge_sort_top_games(top_games: vector<TopGame2048>): vector<TopGame2048> {
        let top_games_length = vector::length(&top_games);
    ...
    public(friend) fun merge(left: vector<TopGame2048>, right: vector<TopGame2048>): vector<TopGame2048> {
        vector::reverse(&mut left);
        vector::reverse(&mut right);

        let result = vector<TopGame2048>[];
    ...
    #[test_only]
    public fun top_game(scenario: &mut Scenario, leader_address: address, top_tile: u64, score: u64): TopGame2048 {
        let ctx = test_scenario::ctx(scenario);
