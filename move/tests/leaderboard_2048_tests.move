...
#[test_only]
module ethos::leaderboard_2048_tests {
    use std::option;
    use std::vector;
    ...
    use ethos::game_board_2048::{left, up, right, down};

    use ethos::leaderboard_2048::{Self, Leaderboard2048};
    use ethos::game_2048::{Self, Game2048, Game2048Maintainer};
    ...
    fun create_game(scenario: &mut Scenario) {
        ...
        let maintainer = game_2048::create_maintainer(ctx);
        ...
        game_2048::create(&mut maintainer, coins, ctx);

        sui::test_utils::destroy<Game2048Maintainer>(maintainer);
    }
    ...
    fun achieve_score(scenario: &mut Scenario, score: u64) {
        ...
        test_scenario::next_tx(scenario, PLAYER);
        {
            let leaderboard = test_scenario::take_shared<Leaderboard2048>(scenario);
            let game = test_scenario::take_from_sender<Game2048>(scenario);
        
            let ctx = test_scenario::ctx(scenario);
            while (*game_2048::score(&game) < score) {
                game_2048::make_move(&mut game, left(), ctx);
                game_2048::make_move(&mut game, up(), ctx);
                game_2048::make_move(&mut game, right(), ctx);
                game_2048::make_move(&mut game, down(), ctx);
            };

            leaderboard_2048::submit_game(&mut game, &mut leaderboard);
    ...
    fun achieve_score_game(scenario: &mut Scenario, game_id: ID, score: u64) {
        ...
        {
            let leaderboard = test_scenario::take_shared<Leaderboard2048>(scenario);
            let game = test_scenario::take_from_sender_by_id<Game2048>(scenario, game_id);
    ...
    fun check_scores_for_leaderboard(leaderboard: &Leaderboard2048, scores: vector<u64>) {
        let index = 0;
        while (index < vector::length(&scores)) {
            let top_game = leaderboard_2048::top_game_at(leaderboard, index);
            assert!(leaderboard_2048::top_game_score(top_game) == vector::borrow(&scores, index), index);
            index = index + 1;
        };
        let top_games_length = vector::length(leaderboard_2048::top_games(leaderboard));
    ...
    fun test_submit_game() {
        ...
        leaderboard_2048::create(test_scenario::ctx(&mut scenario)); 

        achieve_score(&mut scenario, 8);

        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            let leaderboard = test_scenario::take_shared<Leaderboard2048>(&mut scenario);
            
            let game = test_scenario::take_from_sender<Game2048>(&mut scenario);
            
            let top_game = leaderboard_2048::top_game_at(&leaderboard, 0);
            assert!(leaderboard_2048::top_game_game_id(top_game) == game_2048::id(&game), 1);
    ...
    fun test_submit_game__inserts_game_at_correct_location_in_top_list() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard_2048::top_game_at(&leaderboard, 0);
            assert!(leaderboard_2048::top_game_game_id(top_game) == game_2048::id(&game), 0);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard_2048::top_game_at(&leaderboard, 1);
            assert!(leaderboard_2048::top_game_game_id(top_game) == game_2048::id(&game), 1);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game = leaderboard_2048::top_game_at(&leaderboard, 3);
            assert!(leaderboard_2048::top_game_game_id(top_game) == game_2048::id(&game), 1);
    ...
    fun test_submit_game__only_one_entry_per_game() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let leaderboard_game_count = vector::length(leaderboard_2048::top_games(&leaderboard));
    ...
    fun test_submit_game__only_one_entry_per_game_with_multiple_games() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let game = test_scenario::take_from_sender_by_id<Game2048>(&mut scenario, game1_id);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let game = test_scenario::take_from_sender_by_id<Game2048>(&mut scenario, game2_id);
    ...
    #[test]
    #[expected_failure(abort_code = leaderboard_2048::ELowScore)]
    fun test_submit_game__aborts_if_not_a_leader() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 2, 0, 0);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let game = test_scenario::take_from_sender<Game2048>(&mut scenario);
            game_2048::make_move(&mut game, up(), test_scenario::ctx(&mut scenario));
            game_2048::make_move(&mut game, left(), test_scenario::ctx(&mut scenario));
            leaderboard_2048::submit_game(&mut game, &mut leaderboard);
    ...
    fun test_submit_game__stores_min_tile_and_min_score() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            assert!(leaderboard_2048::min_tile(&leaderboard) == &0, (*leaderboard_2048::min_tile(&leaderboard) as u64));
            assert!(leaderboard_2048::min_score(&leaderboard) == &0, *leaderboard_2048::min_score(&leaderboard));

            assert!(game_2048::score(&game) == &4, *game_2048::score(&game));

            test_scenario::return_to_sender(&mut scenario, game);
            test_scenario::return_shared<Leaderboard2048>(leaderboard)
        };
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            assert!(leaderboard_2048::min_tile(&leaderboard) == &2, (*leaderboard_2048::min_tile(&leaderboard) as u64));
            assert!(leaderboard_2048::min_score(&leaderboard) == &4, *leaderboard_2048::min_score(&leaderboard));
            
            assert!(game_2048::score(&game) == &16, *game_2048::score(&game));
    ...
    #[test]
    #[expected_failure(abort_code = leaderboard_2048::ELowTile)]
    fun test_submit_game__aborts_if_not_above_min_tile() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 2, 3, 30);
    ...
    fun test_submit_game__aborts_if_not_above_min_score() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 2, 0, 30);
    ...
    fun test_submit_game__accepts_game_if_above_min_score_even_if_leaderboard_full_removes_last_entry() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_games = leaderboard_2048::top_games(&leaderboard);
            let leaderboard_game_count = vector::length(top_games);
            assert!(leaderboard_game_count == 2, leaderboard_game_count);
            
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game3_id == leaderboard_2048::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game2_id == leaderboard_2048::top_game_game_id(top_game1), 1);
    ...
    fun test_submit_game__top_game_removes_bottom() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 3, 0, 0);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            let game4 = test_scenario::take_from_sender<Game2048>(&mut scenario);
            let game3 = test_scenario::take_from_sender<Game2048>(&mut scenario);
            let game2 = test_scenario::take_from_sender<Game2048>(&mut scenario);
            let game1 = test_scenario::take_from_sender<Game2048>(&mut scenario);
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game_2048::id(&game4) == leaderboard_2048::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game_2048::id(&game3) == leaderboard_2048::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(game_2048::id(&game2) == leaderboard_2048::top_game_game_id(top_game2), 2);
    ...
    fun test_submit_game__middle_game_removes_bottom() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game_2048::id(&game3) == leaderboard_2048::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game_2048::id(&game1) == leaderboard_2048::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(game_2048::id(&game4) == leaderboard_2048::top_game_game_id(top_game2), 2);
    ...
    fun test_submit_game__bottom_game_removes_bottom() {
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(game_2048::id(&game1) == leaderboard_2048::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(game_2048::id(&game4) == leaderboard_2048::top_game_game_id(top_game1), 1);
    ...
    fun test_lots_of_games() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 9, 0, 0);
    ...
    fun test_move_one_game_around() {
        let scenario = test_scenario::begin(PLAYER);
        leaderboard_2048::blank_leaderboard(&mut scenario, 5, 0, 0);

        achieve_score(&mut scenario, 88);
        check_scores(&mut scenario, vector<u64>[88]);
        let game_id = test_scenario::most_recent_id_for_address<Game2048>(PLAYER);
        ...
        let game5_id = test_scenario::most_recent_id_for_address<Game2048>(PLAYER);

        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            let leaderboard = test_scenario::take_shared<Leaderboard2048>(&mut scenario);
            let game = test_scenario::take_from_address_by_id<Game2048>(&mut scenario, PLAYER, option::destroy_some(game_id));
        ...
        {
            let leaderboard = test_scenario::take_shared<Leaderboard2048>(&mut scenario);
            let game = test_scenario::take_from_address_by_id<Game2048>(&mut scenario, PLAYER, option::destroy_some(game5_id));
    ...
    fun test_lots_of_players() {
        ...
        achieve_score(&mut scenario, 88);
        check_scores(&mut scenario, vector<u64>[88]);
        let game_1_id = test_scenario::most_recent_id_for_address<Game2048>(PLAYER);
        ...
        let game_2_id = test_scenario::most_recent_id_for_address<Game2048>(PLAYER);
        ...
        let game_3_id = test_scenario::most_recent_id_for_address<Game2048>(PLAYER);
        ...
        test_scenario::next_tx(&mut scenario, PLAYER);
        {
            ...
            let top_game0 = vector::borrow(top_games, 0);
            assert!(option::destroy_some(game_2_id) == leaderboard_2048::top_game_game_id(top_game0), 0);

            let top_game1 = vector::borrow(top_games, 1);
            assert!(option::destroy_some(game_1_id) == leaderboard_2048::top_game_game_id(top_game1), 1);
          
            let top_game2 = vector::borrow(top_games, 2);
            assert!(option::destroy_some(game_3_id) == leaderboard_2048::top_game_game_id(top_game2), 2);

            // leaderboard_2048::reset_leaderboard(&mut leaderboard);
    ...
    fun assert_sort(top_games: vector<leaderboard_2048::TopGame2048>, expected: vector<vector<u64>>) {
        let sorted = leaderboard_2048::merge_sort_top_games(top_games);
        assert_sorted(sorted, expected);
    }

    fun assert_merge(left: vector<leaderboard_2048::TopGame2048>, right: vector<leaderboard_2048::TopGame2048>, expected: vector<vector<u64>>) {
        let merged = leaderboard_2048::merge(left, right);
        assert_sorted(merged, expected);
    }

    fun assert_sorted(sorted: vector<leaderboard_2048::TopGame2048>, expected: vector<vector<u64>>) {
        ...
        while (!vector::is_empty(&sorted)) {
            ...
            assert!(score == *leaderboard_2048::top_game_score(&top_game), score);
            assert!(tile == *leaderboard_2048::top_game_top_tile(&top_game), tile);
        }
    }
    ...
    fun test_sort() {
        ...
        let top_games = vector<leaderboard_2048::TopGame2048>[
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 200),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 500),
        ];
        ...
        let top_games = vector<leaderboard_2048::TopGame2048>[
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 200),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 500),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 400),
        ];
        assert_sort(top_games, vector[
            vector[2, 500], 
            vector[2, 400], 
        ...
        let top_games = vector<leaderboard_2048::TopGame2048>[
            ...
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 500),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 400),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 300),
        ];
        assert_sort(top_games, vector[
            vector[2, 500], 
            vector[2, 400], 
        ...
        let top_games = vector<leaderboard_2048::TopGame2048>[
            leaderboard_2048::top_game(&mut scenario, PLAYER, 1, 200),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 4, 300),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 400),
            leaderboard_2048::top_game(&mut scenario, PLAYER, 3, 500),
        ];
        assert_sort(top_games, vector[
            vector[4, 300], 
            vector[3, 500], 
    ...
    fun test_merge() {
        ...
        let left = vector<leaderboard_2048::TopGame2048>[
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 200),
        ];
        let right = vector<leaderboard_2048::TopGame2048>[
            leaderboard_2048::top_game(&mut scenario, PLAYER, 2, 500),
        ];
